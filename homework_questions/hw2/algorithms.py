...
def bfgs( func, initial_x, initial_inv_h, eps=1e-5, maximum_iterations=65536, linesearch=bisection, *linesearch_args  ):
    ...
    *linesearch_args:   the extra arguments of linesearch routine
    ...
    while True:
        ...
        if np.power(gradient.T*gradient,0.5) <= eps:
            break
        ...
        __,gradient = func( x , 1 )
        gradient = np.matrix(gradient)
        s = x - old_x
        y = gradient - old_gradient
        inv_rho = np.asscalar(y.T*s)
        if inv_rho >= 1e-9:
            rho = 1/inv_rho
            inv_h = (np.identity((rho*(s*y.T)).shape[0])-(rho*(s*y.T)))*inv_h*(np.identity((rho*(y*s.T)).shape[0])-rho*(y*s.T))+rho*(s*s.T)
        iterations += 1
        if iterations >= maximum_iterations:
            break
