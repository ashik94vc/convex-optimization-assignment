...
# Draw contour plots
# hw_func.draw_contour( func, gd_xs, bfgs_xs, 0, levels=np.arange(5, 400, 20), x=np.arange(-5, 5.1, 0.1), y=np.arange(-5, 5.1, 0.1))
...
line_bfgs, = plt.semilogy([x - hw_func.rosenbrock_func([1.0, 1.0]) for x in bfgs_back_values], linewidth=2, color='g', marker='o',label='bfgs')
plt.show()

# Draw contour plots
# hw_func.draw_contour( hw_func.rosenbrock_func, gd_xs, bfgs_xs, 1, levels=np.arange(0, 500, 10), x=np.arange(-2, 2, 0.1), y=np.arange(-2, 2, 0.1))
